...
import { SessionInfo } from "../game/storage/session";
import { Builder } from "../game/multiplayer/loginSystem";
...
export class Stage {
  ...
  public switchTo(builder: (index: number, stage: Stage) => void, index: number) {
    ...
    // Set Default Multiplayer attributes in session storage
    let nextLevel = { nextBuilder: "", playerLimit: 0 }
    if (!stage.storage.getSession("sStore")) { stage.storage.setSession("sStore", new SessionInfo()) };
    let sStore = stage.storage.getSession("sStore") as SessionInfo;
    sStore.nextLevel = nextLevel;

    // If the player limits and builder names are present, set them in persistent storage
    if ((builder as Builder).playerLimit && (builder as Builder).builderName) {
      nextLevel.nextBuilder = (builder as Builder).builderName + index;
      nextLevel.playerLimit = (builder as Builder).playerLimit;
    }

    // Set network handler for when we leave later
    // DO NOT PUT INSIDE IF STATEMENT BELOW. 
    // THIS BREAKS IT FOR SOME REASON.
    stage.network.resLeft = () => {
      let buildInfo = (stage.storage.getSession("sStore") as SessionInfo).nextLevel;
      if (buildInfo) {
        stage.network.doReqSwitch(buildInfo.nextBuilder, buildInfo.playerLimit);
      }
    }
