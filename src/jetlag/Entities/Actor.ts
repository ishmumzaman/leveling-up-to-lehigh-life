...
/** A base class for Actors' "Extra" configuration */
export class Extra {
  /**
   * This is a dummy field.  Its only purpose is to coerce TypeScript into doing
   * proper type checking of Extras, since empty classes are too trivial to
   * extend
   */
  protected isExtra = true;
}
...
export class Actor {
   ...
   */
  public constructor(config: { rigidBody: RigidBodyComponent, appearance: AppearanceComponent | AppearanceComponent[], movement?: MovementComponent, role?: RoleComponent, gestures?: GestureHandlers, sounds?: SoundEffectComponent, onDisappear?: (a: Actor) => void, extra?: Extra }) {
    ...
    this.extra = config.extra ?? new Extra();
  }
