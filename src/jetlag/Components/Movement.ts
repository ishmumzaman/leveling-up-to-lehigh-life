...
export class ChaseMovement {
  ...
  /** is this actor currently chasing the other actor? */
  public set enabled(val: boolean) { this._active = val; }
  public get enabled() { return this._active; }

  /** Change the target to chase */
  public set target(target: Actor) { stage.world.oneTimeEvents.push(() => this._target = target) }
  public get target() { return this._target }

  /** Do any last-minute adjustments related to the movement */
  prerender(_elapsedMs: number, _camera: CameraService) {
    // This code only works if we've got a body, the target has a body, both
    // are enabled, and chasing is active
    if (!this.rigidBody || !this._target.rigidBody) return;
    if (!this._target.enabled) return;
    if (this._active) {
      this.rigidBody.body.SetLinearVelocity({ x: 0, y: 0 });

      // compute vector between actors, and normalize it
      let x = this._target.rigidBody!.body.GetPosition().x - this.rigidBody.body.GetPosition().x;
      let y = this._target.rigidBody!.body.GetPosition().y - this.rigidBody.body.GetPosition().y;
    ...
    if (this.pointTo) {
      let targetLocation = this._target.rigidBody.getCenter();
      let usLocation = this.rigidBody.getCenter();
      let angleDiff = Math.atan2(targetLocation.y - usLocation.y, targetLocation.x - usLocation.x) - this.rigidBody.getRotation();
      // Normalize Angle Diff
      angleDiff = ((angleDiff + Math.PI) % (2 * Math.PI)) - Math.PI;
      if (angleDiff < -Math.PI) angleDiff += 2 * Math.PI;
      let deltaTime = 1 / 45; // 45 FPS

      this.rigidBody?.body.SetAngularVelocity(angleDiff / deltaTime);
    }
  }
  ...
  /** Should it actively chase? */
  private _active: boolean;
  /** Should the actor point to who it's chasing? */
  private pointTo: boolean;
   ...
   *                        chaseInY is false)
   */
  constructor(cfg: { speed: number, target: Actor, chaseInX?: boolean, chaseInY?: boolean, multiplier?: number, pointTo?: boolean }) {
    ...
    this._active = true;
    this.pointTo = cfg.pointTo || false;
  }
}
...
export class ManualMovement {
   ...
   *
   * @param cfg.gravityAffectsIt  Does gravity affect this actor?
   * @param cfg.rotateByDirection Should the actor rotate based on its direction
   *                              of movement?
   */
...
export class NavigationMovement {
